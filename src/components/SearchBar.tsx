...
const SearchBar: React.FC<SearchBarProps> = ({ 
  ...
  return (
    <form 
      ...
      role="search"
      aria-label="Search sports events"
    >
      <Search className={`absolute left-2 top-1/2 transform -translate-y-1/2 h-3 w-3 md:h-4 md:w-4 text-gray-400 pointer-events-none z-10`} aria-hidden="true" />
      <Input 
        type="search" 
        ...
        value={value}
        onChange={onChange}
        aria-label="Search sports events and matches"
      />
      {showButton && (
        <Button 
          type="submit" 
