...
const ManualMatchCard = ({ match }: ManualMatchCardProps) => {
  ...
  return (
    <div 
      className="relative w-full max-w-sm mx-auto h-64 bg-black rounded-lg overflow-hidden cursor-pointer hover:scale-105 transition-transform duration-200 group shadow-lg"
    ...
    >
      ...
      {/* ===== SHADOW OVERLAYS ===== */}
      {/* Top shadow */}
      <div className="pointer-events-none absolute top-0 left-0 w-full h-12 bg-gradient-to-b from-black/60 via-black/20 to-transparent z-20" />
      {/* Bottom shadow */}
      <div className="pointer-events-none absolute bottom-0 left-0 w-full h-16 bg-gradient-to-t from-black/70 via-black/20 to-transparent z-20" />
      ...
      <div className="absolute top-0 left-0 right-0 p-3 flex justify-between items-start z-30">
        ...
        <div className="bg-black/60 backdrop-blur-sm rounded-md px-2 py-1 flex flex-col items-start">
          <div className="text-white text-sm font-bold">{timeString} <span className="text-xs text-gray-400 ml-1">AEST</span></div>
        </div>
        {/* LIVE Badge */}
