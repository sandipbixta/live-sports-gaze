...
import LeagueRegionFilter from '../components/live/LeagueRegionFilter';
...
const Live = () => {
  ...
  useEffect(() => {
    ...
    // Then filter by region if not "all" and sport is football
    if (activeRegionFilter !== "all") {
      matchesToFilter = matchesToFilter.filter(match => {
        // Only apply region filter to football matches
        if (match.sportId !== 'football' && match.category !== 'football') {
          return true; // Keep non-football matches
        }
        
        if (activeRegionFilter === 'european') {
          return isEuropeanLeague(match.title);
        } else if (activeRegionFilter === 'usa') {
          return isUSALeague(match.title);
        }
  ...
  }, [searchQuery, activeTab, activeSportFilter, activeRegionFilter, allMatches, liveMatches, upcomingMatches]);
  ...
  return (
    <PageLayout>
      ...
      <LeagueRegionFilter
        ...
        onRegionFilterChange={setActiveRegionFilter}
      />
