...
const FootballLeagues = () => {
  ...
  // First fetch leagues from database
  const { data: leagues, isLoading, error, refetch } = useQuery({
    queryKey: ['sportsdb-leagues'],
    queryFn: async () => {
  ...
  const handleRefresh = async () => {
    ...
    try {
      // Force fetch new data from TheSportsDB
      await supabase.functions.invoke('fetch-leagues');
      await refetch();
    } catch (err) {
  ...
  // Categorize leagues
  const europeanLeagues = leagues?.filter(l => 
    ['England', 'Spain', 'Germany', 'Italy', 'France', 'Netherlands', 'Portugal', 'Scotland', 'Belgium'].includes(l.country || '')
  ) || [];
  
  const europeanCompetitions = leagues?.filter(l => 
    l.country === 'Europe' || l.league_name.includes('UEFA')
  ) || [];
  
  const americanLeagues = leagues?.filter(l => 
    ['USA', 'Brazil', 'Argentina', 'South America'].includes(l.country || '')
  ) || [];
  
  const internationalCompetitions = leagues?.filter(l => 
    ['World', 'Africa'].includes(l.country || '') || l.league_name.includes('World Cup') || l.league_name.includes('Cup of Nations')
  ) || [];
  
  const asianLeagues = leagues?.filter(l => 
    ['Saudi Arabia', 'Japan', 'South Korea'].includes(l.country || '')
  ) || [];
  ...
  const LeagueSection = ({ title, icon, leagues }: { title: string; icon: React.ReactNode; leagues: League[] }) => {
